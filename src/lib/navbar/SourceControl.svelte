...
{#if focus.sources != null}
    {#if tableBlocks.filter((table) => table.title == focus.sources)[0].dataType == 'Quantitative' || tableBlocks.filter((table) => table.title == focus.sources)[0].dataType == 'Binary' || focus.testType == 'X2GOFTest'}
        <ColumnSelect value={col} setter={setCol} num="1">
            {#each tableBlocks.filter((table) => table.title == focus.sources)[0].content[0] as title}
