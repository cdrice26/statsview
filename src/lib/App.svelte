<script>
    ...
    const redo = () => {
        if (redoStack.length > 0) {
            ...
            undoStack = [
                structuredClone($state.snapshot(blocks)),
                ...undoStack,
            ];
            if (undoStack.length > 50) {
                undoStack.pop();
    ...
    const getFromCSV = async (file) => {
        if (file) {
            ...
            blocks = blocks.map((block) => {
                if (block.id == focusedId) {
                    block.content = data;
                    block.dataType = new Array(data[0].length).fill(
                        "Categorical",
                    );
                }
                return block;
    ...
    const delBlock = () => {
        ...
        blocks = blocks
            ...
            .map((block) => {
                if (block?.sources === focusedBlock?.title)
                    block.sources = null;
                return block;
            });
    };
